...
**Default:**  
`docker run --rm -ti -v $(pwd):/build webcoastdk/composer:1.8-php7.0-alpine {composerCommand}`

**SSH forwarding:**  
`docker run --rm -ti -v $(pwd):/build -v $SSH_AUTH_SOCK:$SSH_AUTH_SOCK -e SSH_AUTH_SOCK=$SSH_AUTH_SOCK webcoastdk/composer:1.8-php7.0-alpine {composerCommand}`

**SSH agent:**
`docker run --rm -ti -v $(pwd):/build -v ~/.ssh:/root/.ssh webcoastdk/composer:1.8-php7.0-alpine {composerCommand}`
...
services:
  composer:
    image: webcoastdk/composer:1.8-php7.0-alpine
    volumes:
      - ./:/build
```
