...
**Default:**  
`docker run --rm -ti -v $(pwd):/build webcoastdk/composer:2.0-php7.1-stretch {composerCommand}`

**SSH forwarding:**  
`docker run --rm -ti -v $(pwd):/build -v $SSH_AUTH_SOCK:$SSH_AUTH_SOCK -e SSH_AUTH_SOCK=$SSH_AUTH_SOCK webcoastdk/composer:2.0-php7.1-stretch {composerCommand}`

**SSH agent:**
`docker run --rm -ti -v $(pwd):/build -v ~/.ssh:/root/.ssh webcoastdk/composer:2.0-php7.1-stretch {composerCommand}`
...
services:
  composer:
    image: webcoastdk/composer:2.0-php7.1-stretch
    volumes:
      - ./:/build
```
