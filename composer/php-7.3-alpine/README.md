...
**Default:**  
`docker run --rm -ti -v $(pwd):/build webcoastdk/composer:php-7.3-alpine {composerCommand}`

**SSH forwarding:**  
`docker run --rm -ti -v $(pwd):/build -v $SSH_AUTH_SOCK:$SSH_AUTH_SOCK -e SSH_AUTH_SOCK=$SSH_AUTH_SOCK webcoastdk/composer:php-7.3-alpine {composerCommand}`

**SSH agent:**
`docker run --rm -ti -v $(pwd):/build -v ~/.ssh:/root/.ssh webcoastdk/composer:php-7.3-alpine {composerCommand}`
...
services:
  composer:
    image: webcoastdk/composer:php-7.3-alpine
    volumes:
      - ./:/build
```
