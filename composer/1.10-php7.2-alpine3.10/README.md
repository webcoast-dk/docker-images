...
**Default:**  
`docker run --rm -ti -v $(pwd):/build webcoastdk/composer:1.10-php7.2-alpine3.10 {composerCommand}`

**SSH forwarding:**  
`docker run --rm -ti -v $(pwd):/build -v $SSH_AUTH_SOCK:$SSH_AUTH_SOCK -e SSH_AUTH_SOCK=$SSH_AUTH_SOCK webcoastdk/composer:1.10-php7.2-alpine3.10 {composerCommand}`

**SSH agent:**
`docker run --rm -ti -v $(pwd):/build -v ~/.ssh:/root/.ssh webcoastdk/composer:1.10-php7.2-alpine3.10 {composerCommand}`
...
services:
  composer:
    image: webcoastdk/composer:1.10-php7.2-alpine3.10
    volumes:
      - ./:/build
```
