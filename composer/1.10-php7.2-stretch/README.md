...
**Default:**  
`docker run --rm -ti -v $(pwd):/build webcoastdk/composer:1.10-php7.2-stretch {composerCommand}`

**SSH forwarding:**  
`docker run --rm -ti -v $(pwd):/build -v $SSH_AUTH_SOCK:$SSH_AUTH_SOCK -e SSH_AUTH_SOCK=$SSH_AUTH_SOCK webcoastdk/composer:1.10-php7.2-stretch {composerCommand}`

**SSH agent:**
`docker run --rm -ti -v $(pwd):/build -v ~/.ssh:/root/.ssh webcoastdk/composer:1.10-php7.2-stretch {composerCommand}`
...
services:
  composer:
    image: webcoastdk/composer:1.10-php7.2-stretch
    volumes:
      - ./:/build
```
